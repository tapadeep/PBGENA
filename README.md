...
```
The system must have [Open MPI](https://www.open-mpi.org/), [MPICH](https://www.mpich.org/), or [Microsoft MPI](https://docs.microsoft.com/en-us/message-passing-interface/microsoft-mpi) installed.
...
Except for MAKG all of the above reported results were performed with ```N=2000```, ```tr=0.7```, ```erf=0.3```, and ```p=32``` in a server with 270GB RAM with proper [hyperparameters](https://github.com/tapadeep/PBGENA#relevant-hyperparameters). For MAKG, we used ```tr=0.1```, ```p=6```, and ```f=100```. To get even better results perform the experiments at ```N=8000```. A more comprehensive performance report can be viewed [here](https://docs.google.com/spreadsheets/d/1jpYXgJpBGchIBqaAZZ0CTGMWw-1svk9srWlS49qo7GQ/edit?usp=sharing).
