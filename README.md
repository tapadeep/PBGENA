...
Except for MAKG all of the above reported results were performed with ```N=2000```, ```tr=0.7```, ```erf=0.3```, and ```p=32``` in a server with 270GB RAM. For MAKG, we used ```tr=0.1```, ```p=6```, and ```f=100```. To get even better results perform the experiments at ```N=8000```.
