...
class LinkPredictor(object):
    def __init__(self,graph,algorithm,erf,**kwargs):
        ...
        if self.__algorithm=='PBGENA':
            ...
            self.__b_a=kwargs['b_a']
            self.__p=kwargs['p']
            self.__model=PBGENA.PBGENA(graph=self.__graph,p=self.__p,N=self.__N,alpha=self.__alpha,b_t=self.__b_t,b_a=self.__b_a)
...
if __name__=='__main__':
    ...
    link_predictor=LinkPredictor(graph=args.graph,algorithm=args.algorithm,erf=args.erf,N=args.N,alpha=args.alpha,b_t=args.b_t,b_a=args.b_a,l_t=args.l_t,l_a=args.l_a,f_t=args.f_t,f_a=args.f_a,p=args.p)
