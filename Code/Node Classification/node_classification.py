...
class NodeClassifier(object):
    def __init__(self,graph,algorithm,multi,tr):
        ...
        assert os.path.isfile('../../Embeddings/'+graph+'_'+algorithm+'_emb.pkl'),'Either network name {0} or algorithm name {1} is wrong'.format(self.__graph,algorithm)
        self.__algorithm=algorithm
        self.__emb=pickle.load(open('../../Embeddings/'+graph+'_'+self.__algorithm+'_emb.pkl','rb'))
...
if __name__=='__main__':
    ...
    parser.add_argument('--multi',type=bool,default=False,help='Multi-labelled?')
