...
comm.Bcast(Phi)
Phi_inverse=np.zeros(nodes+1,dtype=int)
j=0
...
if N_t>0:
    ...
    for i in cross_edges:
        S_t[i[0]][Pi_t[i[1]]]=1
    del(Pi_t)
    E_t={i:S_t[i] for i in Phi_inverse}
    D_t=dict()
    for i in Phi_inverse:
...
if N_a>0:
    ...
    E_a={i:S_a[i] for i in Phi_inverse}
    D_a=dict()
    for i in Phi_inverse:
...
if N_t>0:
    ...
    for i in cross_edges:
        batch_D_t[Phi[i[1]]][i[0]]=D_t[i[0]]
if N_a>0:
    batch_D_a=[dict() for _ in range(size)]
    for i in cross_edges:
        batch_D_a[Phi[i[1]]][i[0]]=D_a[i[0]]
del(Phi)
if N_t>0:
    D_t=dict()
    if f==1:
